...
def perform_distance_reduction_analysis(df1 : pd.DataFrame, df2 : pd.DataFrame, mask: pd.Series, filename: str):
    df1_reduced = df1[mask]
    df2_reduced = df2[mask]
    reduced_distance = df1_reduced['l1_dist'] - df2_reduced['l1_dist']
    reduction_ratio = reduced_distance / df1_reduced['l1_dist']
...
def main():
    filename = "exp_counterfactual_distance.txt"
    for name in known_datasets:
        ...
        # IPS-KNN === k-NN
        base_y_pred, _, _, _, _, cf_base, _ =  classifier_ips_knn.predict_with_explanation(dataset.X_test, False, False, True, False, False)
        cf_base = cf_base.groupby(level='index').first()
        
        # E-IPS-KNN-T
        custom_print('E-IPS-KNN T: ******************************************************************', filename)
        t_y_pred, _, _, _, _, cf_t = classifier_e_ips_knn_t.predict_with_explanation(dataset.X_test, False, False, True, False)
        cf_t = cf_t.groupby(level='index').first()
        mask = (base_y_pred == t_y_pred).sort_index()
        perform_distance_reduction_analysis(cf_base, cf_t, mask, filename)

        # E-IPS-KNN-T&F
        custom_print('E-IPS-KNN T&F: ****************************************************************', filename)
        tf_y_pred, _, _, _, _, cf_tf = classifier_e_ips_knn_tf.predict_with_explanation(dataset.X_test, False, False, True, False)
        cf_tf = cf_tf.groupby(level='index').first()
        mask = (base_y_pred == tf_y_pred).sort_index()
        perform_distance_reduction_analysis(cf_base, cf_tf, mask, filename)
